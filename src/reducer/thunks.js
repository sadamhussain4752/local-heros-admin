...
const UpdateOrderstatus = `${constant.baseUrl}api/order/orderUpdate`;
...
export const OrderUpdate = (id, body) => async (dispatch) => {
  dispatch(OrderStatusRequest());

  try {
    // Send the POST request with the provided body data
    const response = await axios.put(`${UpdateOrderstatus}/${id}`, body);
    dispatch(OrderStatusSuccess(response.data));
  } catch (error) {
    console.log(error);
    dispatch(OrderStatusFailure(error.response.data.message));
  }
};
...
export const OrderRefresh = (id) => async (dispatch) => {
  console.log(id);
  dispatch(OrderRequestRefresh(id));
};
